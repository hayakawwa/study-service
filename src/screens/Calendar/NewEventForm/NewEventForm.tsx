...
const NewEventForm: FC<NewEventProps> = ({ onDate, onName, onFinish }) => {
	...
	// @ts-ignore
	const coursesNames = useAppSelector(getCoursesNames);
	...
	const coursesNamesList: CoursesSchema[] = [];
	coursesNames &&
		coursesNames.forEach((course) => {
			coursesNamesList.push({ value: course, label: course });
		});
